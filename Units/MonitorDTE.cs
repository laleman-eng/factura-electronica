    ...
    public class TMonitorDTE : TvkBaseForm, IvkFormInterface
    {
        ...
        public new bool InitForm(string uid, string xmlPath, ref SAPbouiCOM.Application application, ref SAPbobsCOM.Company company, ref CSBOFunctions SBOFunctions, ref TGlobalVid _GlobalSettings)
        {
            ...
            try
            {
                ...
                if (MenuLlamado == "Ventas")
                    oComboBox.Select("V", BoSearchKey.psk_ByValue);
                else
                    oComboBox.Select("C", BoSearchKey.psk_ByValue);
                //Bloquear Combo TipoDOC
        ...
        public new void FormEvent(String FormUID, ref SAPbouiCOM.ItemEvent pVal, ref Boolean BubbleEvent)
        {
            ...
            try
            {
                if ((pVal.EventType == BoEventTypes.et_CLICK) && (!pVal.BeforeAction) && (pVal.ItemUID == "grid"))
                {
                    oGrid.Rows.SelectedRows.Add(pVal.Row);
                }

                if ((pVal.EventType == BoEventTypes.et_MATRIX_LINK_PRESSED) && (pVal.BeforeAction) && (pVal.ItemUID == "grid") && (pVal.ColUID == "DocEntryDoc"))
                {
                    string TypeObj = oGrid.DataTable.GetValue("U_ObjType", pVal.Row).ToString();
                    string FactNum = oGrid.DataTable.GetValue("DocCompra", pVal.Row).ToString();
                    switch (TypeObj)
                    {
                        case "112":
                            if (FactNum.Trim().Length > 0 && FactNum != "0")
                                ((EditTextColumn)oGrid.Columns.Item("DocEntryDoc")).LinkedObjectType = "18";
                            else
                                ((EditTextColumn)oGrid.Columns.Item("DocEntryDoc")).LinkedObjectType = "112";
                            break;
                        case "18":
                            ((EditTextColumn)oGrid.Columns.Item("DocEntryDoc")).LinkedObjectType = "18";
                            break;
        ...
        private void BuscarDatos()
        {
            ...
            try
            {
                ...
                if (TipoDTE == "V") //VENTAS
                {
                    if (GlobalSettings.RunningUnderSQLServer)
                        ...
                        s = @"SELECT T0.""DocEntry""
                         ...
                         WHERE 1 = 1
                           AND T0.""{3}"" BETWEEN '{0}' AND '{1}'";
                    s = String.Format(s, FechaD, FechaH, "@VID_FEDTEVTA", (DSOpFec.Value == "1" ? "U_FechaEmi" : "U_FechaRecep"), (TipoDTE == "C" ? (GlobalSettings.RunningUnderSQLServer ? @"T0.U_Xml" : @"T0.""U_Xml"" ") : "''"));
                }
                else
                {
                    if (GlobalSettings.RunningUnderSQLServer)
                        s = @"SELECT T0.DocEntry
	                          ...
	                          ,T0.U_EstadoSII EstadoSII
	                          ,T0.U_EstadoLey EstadoLey
                              ,CASE WHEN T0.U_EstadoSII = 'P' THEN NULL ELSE CAST(T0.U_DocEntry AS INT)  END DocEntryDoc
                              ,{4} 'xml'  
                              ,T0.U_ObjType
							  ,T1.DocEntry  DocCompra
                          FROM [{2}] T0
                               LEFT JOIN OPCH T1 ON T0.U_Folio = T1.FolioNum AND T0.U_RUT = REPLACE(T1.LicTradNum,'.','') AND T1.CANCELED = 'N'
                         WHERE 1 = 1
                           AND T0.{3} BETWEEN '{0}' AND '{1}'";
                    else
                        s = @"SELECT T0.""DocEntry""
	                          ...
	                          ,T0.""U_EstadoSII"" ""EstadoSII""
	                          ,T0.""U_EstadoLey"" ""EstadoLey""
                              ,CASE WHEN T0.""U_EstadoSII"" = 'P' THEN NULL ELSE TO_INT(T0.""U_DocEntry"") END ""DocEntryDoc""
                              ,{4} ""xml""  
                              ,T0.""U_ObjType""
							  ,T1.""DocEntry""  ""DocCompra""
                          FROM ""{2}"" T0
                         ...
                         WHERE 1 = 1
                           AND T0.""{3}"" BETWEEN '{0}' AND '{1}'";
                    s = String.Format(s, FechaD, FechaH, "@VID_FEDTECPRA", (DSOpFec.Value == "1" ? "U_FechaEmi" : "U_FechaRecep"), (TipoDTE == "C" ? (GlobalSettings.RunningUnderSQLServer ? @"T0.U_Xml" : @"T0.""U_Xml"" ") : "''"));
                }
                ...
                for (int i = 0; i <= oGrid.DataTable.Rows.Count - 1; i++)
                {
                    if (TipoDTE != "V")
                    {
                        string TypeObj = oGrid.DataTable.GetValue("U_ObjType", i).ToString();
                        string FactNum = oGrid.DataTable.GetValue("DocCompra", i).ToString();
                        string EstadoSII = oGrid.DataTable.GetValue("EstadoSII", i).ToString();

                        if (TypeObj == "112" && FactNum.Trim().Length > 0 && FactNum != "0" && EstadoSII != "P")
                            oGrid.DataTable.SetValue("DocEntryDoc", i, FactNum);
                    }
                ...
                ((EditTextColumn)oGrid.Columns.Item("U_ObjType")).Visible = false;
                ((EditTextColumn)oGrid.Columns.Item("DocCompra")).Visible = false;
