    ...
    public class TAsignarFolios : TvkBaseForm, IvkFormInterface
    {
                ...
                if (pVal.EventType == BoEventTypes.et_VALIDATE && pVal.BeforeAction)
                {
                    if (pVal.ItemUID == "mtx")
                    {
                       ...
                       // OutLog((System.String)(oDBDSD.GetValue("U_Hasta", pVal.Row-1)).Trim());
                        int desdeCAF = Convert.ToInt32((System.String)(oDBDSH.GetValue("U_Desde", 0)).Trim());
                        int hastaCAF = Convert.ToInt32((System.String)(oDBDSH.GetValue("U_Hasta", 0)).Trim());
                        if (pVal.ColUID == "Desde")
                        {
                            sHasta = (System.String)(oDBDSD.GetValue("U_Hasta", pVal.Row-1)).Trim();

                            //int desde =    Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Desde", pVal.Row-1)).Trim());
                            ...
                            else if (Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Desde", pVal.Row - 1)).Trim()) != 0 && Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Desde", pVal.Row - 1)).Trim()) < desdeCAF)
                            {
                                FSBOApp.StatusBar.SetText("Folios Desde no puede ser menor al rango del CAF", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                                BubbleEvent = false;
                            }
                            else if (Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Desde", pVal.Row - 1)).Trim()) > hastaCAF)
                            {
                                FSBOApp.StatusBar.SetText("Folios Desde no puede ser mayor al rango del CAF", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                                BubbleEvent = false;
                            }
                            else
                            {
                        ...
                        if (pVal.ColUID == "Hasta")
                        {
                            ...
                            else if (Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Hasta", pVal.Row - 1)).Trim()) < desdeCAF && Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Hasta", pVal.Row - 1)).Trim()) != 0)
                            {
                                FSBOApp.StatusBar.SetText("Folios Hasta no puede ser menor al rango del CAF", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                                BubbleEvent = false;
                            }
                            else if (Convert.ToInt32((System.String)(oDBDSD.GetValue("U_Hasta", pVal.Row - 1)).Trim()) > hastaCAF)
                            {
                                FSBOApp.StatusBar.SetText("Folios Hasta no puede ser mayor al rango del CAF", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                                BubbleEvent = false;
                            }
                            else
                            {
        ...
        private Boolean ValidarManual(Int32 iRow, String sColumna)
        {
            ...
            catch(Exception e)
            {
                ...
                OutLog("ValidarManual: " + e.Message + " ** Trace: " + e.StackTrace);
                return false;
            }
        }//fin ValidarManual
