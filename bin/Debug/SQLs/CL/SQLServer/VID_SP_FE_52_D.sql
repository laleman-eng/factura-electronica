IF EXISTS(SELECT name FROM sysobjects                
      WHERE name = 'VID_SP_FE_52_D' AND type = 'P')
   DROP PROCEDURE VID_SP_FE_52_D
GO--

CREATE PROCEDURE [dbo].[VID_SP_FE_52_D]
(
	 @DocEntry			Int
	,@TipoDoc			VarChar(10)
	,@ObjType			VarChar(10)
...
SELECT   ROW_NUMBER() OVER(ORDER BY T0.LineaOrden, T0.LineaOrden2)								[NroLinDet] 
	...
	FROM	  VID_VW_FE_52_D	   T0
	LEFT JOIN VID_VW_FE_52_D_EXTRA T1 ON T0.DocEntry    = T1.DocEntry
