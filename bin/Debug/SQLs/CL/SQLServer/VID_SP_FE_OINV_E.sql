IF EXISTS(SELECT name FROM sysobjects                
      WHERE name = 'VID_SP_FE_OINV_E' AND type = 'P')
   DROP PROCEDURE VID_SP_FE_OINV_E
GO--

CREATE  PROCEDURE [dbo].[VID_SP_FE_OINV_E]
(
     @DocEntry	Int
    ,@TipoDoc	VarChar(10)
    ,@ObjType	VarChar(10)
...
BEGIN
	SELECT --IdDoc
		...
		,T0.Patente															[Patente]
		,T0.RutChofer														[RutChofer]
		,T0.NombreChofer													[NombreChofer]
		...
		,ISNULL(T0.OtraMoneda,0.0)											[MntExeOtrMnda]
		,ISNULL(T0.OtraMoneda,0.0)											[MntTotOtrMnda]
		--Activo Fijo
