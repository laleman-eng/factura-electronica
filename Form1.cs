...
namespace Factura_Electronica_VK
{
    ...
    public partial class Form1 : System.Windows.Forms.Form
    {
        ...
        void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs args)
        {
            ...
            {
                MainClass.oLog.OutLog(String.Format("CurrentDomain_UnhandledException {0} Trace {1}", e.Message, e.StackTrace));
            }
            else
            {
        ...
        public void Application_ThreadException(object sender, ThreadExceptionEventArgs args)
        {
            if ((MainClass != null) && (MainClass.oLog != null))
            {
                MainClass.oLog.OutLog(String.Format("Application_ThreadException {0} Trace {1}", args.Exception.Message, args.Exception.StackTrace));
            }
            else
            {
    ...
    public class TMainClassExt : TMainClass //class (TMainClass)
    {
        ...
        public override void initApp()
        {
            ...
            try
            {
                ...
                oLog.LogFile = System.IO.Path.GetDirectoryName(TMultiFunctions.ParamStr(0)) + "\\VDx.log";
