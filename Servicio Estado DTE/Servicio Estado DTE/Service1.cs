    ...
    public partial class Service1 : ServiceBase
    {
        ...
        public void ConsultarEstado36(String UserWS, String PassWS, String CompnyName)
        {
            ...
            try
            {
                ...
                foreach (var item in results["Table"])
                {
                    try
                    {
                        string TIPODOC = item.TIPODOC.Value;//"33   
                        Double FOLIO = Convert.ToDouble(item.FOLIO.Value);   //9698123.0
            
                        string RUTEMISOR = item.RUTEMISOR.Value;//"96556940-5"
                        Double MONTO_TOTAL = Convert.ToDouble(item.MONTO_TOTAL.Value);//546256
                        Double IVA = Convert.ToDouble(item.IVA.Value);// 87217.0,
                        Descrip = "Creado por Servicio Estado DTE";
                        FECHARECEPSII = Convert.ToDateTime(item.FECHARECEPSII.Value);
                        FECHAEMIS = Convert.ToDateTime(item.FECHAEMIS.Value);
                        FECHAVENC = Convert.ToDateTime(item.FECHAVENC.Value);// "2017-09-13T00:00:00"
                        Double MONTO_EXENTO = Convert.ToDouble(item.MONTO_EXENTO.Value); // 0.0,
                        Double DESCUENTO_GLOBAL = Convert.ToDouble(item.DESCUENTO_GLOBAL.Value);// 0.0000,
                        string RAZON_SOCIAL = item.RZN_SOCIAL_EMISOR.Value;
                        string OC = (item.OC.Value == null ? "" : item.OC.Value);
                        string NV = (item.NV.Value == null ? "" : item.NV.Value);
                        ...
                        { 
                            ...
                            else
                            {
                                ...
                                var ss = @"http://portal.easydoc.cl//Consulta/GeneracionDte.aspx?RUT1={0}&FOLIO={1}&TIPODTE={2}&RUT={3}&OP=37";
        ...
        public Boolean ConectarBaseSAP(String BaseName)
        {
            ...
            try
            {
               ...
               //oCompany.UseTrusted = false;
               //oCompany.Server = "hanab1-vd:30015";
               //oCompany.DbUserName = "SYSTEM";
               //oCompany.DbPassword = "Soportevk1";
             
               //oCompany.CompanyDB = "VISUALK_CL";
